...
from oneagent.codex_sessions import CodexSessionState, codex_sessions_path, load_codex_session, save_codex_session
from oneagent.identity import load_identity
from oneagent.last_codex_input import last_codex_input_message
from oneagent.memory.store import remember_memory
...
class OneagentBrainTests(unittest.TestCase):
    ...
    @patch.dict("os.environ", {"ONEAGENT_CODEX_MODEL": "gpt-5-codex"}, clear=True)
    def test_model_summary_reports_configured_model(self) -> None:
        summary = model_summary()

        self.assertIn("AI model: gpt-5-codex", summary)
        self.assertIn("Model source: ONEAGENT_CODEX_MODEL", summary)
    ...
    def test_model_summary_keeps_reasoning_when_model_env_overrides_config(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            with patch.dict(
                "os.environ",
                {"CODEX_HOME": temp, "ONEAGENT_CODEX_MODEL": "gpt-5-codex"},
                clear=True,
            ):
                summary = model_summary()
    ...
    def test_model_summary_prefers_oneagent_config_over_codex_config_per_setting(self) -> None:
    ...
    @patch("oneagent.brain.codex_model_options")
    ...
    def test_model_summary_reports_oneagent_model_override(self) -> None:
        with TemporaryDirectory() as temp, TemporaryDirectory() as codex_home:
            root = Path(temp)
            config = root / ".oneagent" / "config.yaml"
            config.parent.mkdir()
            config.write_text(
                "\n".join(["codex:", "  model: gpt-oneagent-local"]),
                encoding="utf-8",
            )
            (Path(codex_home) / "config.toml").write_text(
                'model = "gpt-global"\n',
    ...
    @patch("oneagent.brain.shutil.which", return_value="/usr/local/bin/codex")
    @patch("oneagent.brain.subprocess.run")
    def test_respond_passes_oneagent_reasoning_effort_to_codex(
        self, run: MagicMock, _which: MagicMock
    ) -> None:
        run.return_value.returncode = 0
        run.return_value.stdout = ""
    ...
    @patch("oneagent.brain.shutil.which", return_value="/usr/local/bin/codex")
    @patch("oneagent.brain.subprocess.run")
    def test_respond_passes_oneagent_model_to_codex(
        self, run: MagicMock, _which: MagicMock
    ) -> None:
        run.return_value.returncode = 0
        run.return_value.stdout = ""
        ...
        self.assertEqual(args[args.index("--model") + 1], "gpt-oneagent-local")
    ...
    @patch("oneagent.brain.shutil.which", return_value="/usr/local/bin/codex")
    @patch("oneagent.brain.memory_for_prompt", return_value="Identity and long-term memory.")
    @patch("oneagent.brain.subprocess.run")
    ...
    @patch("oneagent.brain.shutil.which", return_value="/usr/local/bin/codex")
    @patch("oneagent.brain.memory_for_prompt", return_value="Old memory should not be sent.")
    @patch("oneagent.brain.subprocess.run")
    ...
    def test_act_in_session_separates_working_directory_from_agent_state(
        ...
        with TemporaryDirectory() as temp:
            ...
            state_root.mkdir()
            work_root.mkdir()
            config = state_root / ".oneagent" / "config.yaml"
            config.parent.mkdir()
            config.write_text(
                'codex:\n  model: "gpt-5.6-sol"\n  reasoning_effort: "high"\n',
                encoding="utf-8",
        ...
        self.assertIsNotNone(state)
        self.assertFalse((work_root / ".oneagent").exists())
    ...
    @patch.dict("os.environ", {}, clear=True)
    @patch("oneagent.brain.shutil.which", return_value="/usr/local/bin/codex")
    def test_oneagent_config_codex_executable_precedes_path(self, _which: MagicMock) -> None:
        with TemporaryDirectory() as temp:
            root = Path(temp)
    ...
    def test_environment_codex_executable_precedes_oneagent_config(self, _which: MagicMock) -> None:
        with TemporaryDirectory() as temp:
            ...
            with patch.dict(
                "os.environ",
                {"ONEAGENT_CODEX_BIN": str(environment)},
                clear=True,
            ):
                resolution = brain.resolve_codex_executable(root)

        self.assertEqual(resolution.path, str(environment))
        self.assertEqual(resolution.source, "ONEAGENT_CODEX_BIN")
    ...
    @patch("oneagent.brain.time.monotonic", side_effect=[0, 1])
    @patch("oneagent.brain.tempfile.TemporaryFile")
    @patch("oneagent.brain.subprocess.Popen")
    ...
    @patch("oneagent.brain.time.sleep")
    @patch("oneagent.brain.time.monotonic", side_effect=[0, 1])
    @patch("oneagent.brain.tempfile.TemporaryFile")
    @patch("oneagent.brain.subprocess.Popen")
