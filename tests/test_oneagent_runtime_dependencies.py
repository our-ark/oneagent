...
from oneagent.runtime_dependencies import (
...
class OneagentRuntimeDependencyTests(unittest.TestCase):
    ...
    def test_installs_missing_dependency_once_into_private_state(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            with patch(
                "oneagent.runtime_dependencies._pip_install",
                side_effect=install,
            ) as pip:
            ...
            self.assertTrue(
                str(first[0]).startswith(str((root / ".oneagent" / "dependencies").resolve()))
            )
    ...
    def test_uses_explicit_preloaded_dependency_during_birth_validation(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            ), patch("oneagent.runtime_dependencies._pip_install") as pip:
                paths = runtime_dependency_paths(root)
    ...
    def test_skips_missing_optional_dependency_without_installing(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            with patch("oneagent.runtime_dependencies._pip_install") as pip:
                paths = runtime_dependency_paths(root)
