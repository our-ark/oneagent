...
from oneagent.extensions import ExtensionScheduleSpec
from oneagent.extensions.schedules import reconcile_extension_schedules
...
class OneagentPrivateStateTests(unittest.TestCase):
    def test_portable_self_is_registered_and_schema_validated(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self_file = root / ".oneagent" / "self.json"
    ...
    def test_future_file_schema_is_rejected_without_mutation(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            queue = root / ".oneagent" / "task_queue.json"
            queue.parent.mkdir()
            original = json.dumps(
                {
                    "schema_version": 99,
    ...
    def test_future_manifest_is_rejected(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            manifest = root / ".oneagent" / MANIFEST_NAME
    ...
    def test_corrupt_state_is_preserved_and_not_backed_up(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertFalse((root / ".oneagent" / "backups").exists())
    ...
    def test_failed_manifest_commit_restores_migrated_files(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            from oneagent import private_state
            ...
            with patch("oneagent.private_state.atomic_write", side_effect=fail_manifest):
            ...
            self.assertEqual(queue.read_text(encoding="utf-8"), original)
            self.assertFalse(private_state_manifest_path(root).exists())
            self.assertEqual(len(list((root / ".oneagent" / "backups").iterdir())), 1)

    def test_failed_post_migration_validation_rolls_back(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            last_input = root / ".oneagent" / "last_codex_input.json"
            ...
            with patch(
                "oneagent.private_state._current_manifest",
    ...
    def test_live_daemon_blocks_apply_but_not_dry_run(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            epoch = root / ".oneagent" / "daemon_epoch.json"
            epoch.parent.mkdir()
            epoch.write_text(
                json.dumps(
                    {
    ...
    def test_artifact_files_are_outside_private_state_migration(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            artifact = root / ".oneagent" / "artifacts" / "task_events.jsonl"
