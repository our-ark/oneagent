...
class OneAgentGithubWorkflowTests(unittest.TestCase):
    ...
    @patch("our_ark_github.workflow.current_branch", return_value="feature/oneagent")
    ...
    def test_stages_and_commits_with_existing_validation_attestation(
        ...
        self.assertEqual(result.branch, "feature/oneagent")
    ...
    @patch(
        ...
        return_value="https://github.com/our-ark/oneagent/compare/main...feature/oneagent",
    )
    @patch(
    ...
    @patch(
        ...
        side_effect=PublishError(
            "feature/oneagent has no local commits ahead of origin/feature/oneagent."
        ),
    )
    ...
    def test_pushes_current_branch_and_returns_compare_url(
        ...
        self.assertEqual(
            result.compare_url,
            "https://github.com/our-ark/genesis/compare/main...feature/oneagent?expand=1",
        )
        run_git.assert_any_call(["push", "-u", "origin", "feature/oneagent"], ROOT)
    ...
    def test_push_compare_url_allows_dots_in_repo_name(
        ...
        run_git.side_effect = [
            ...
            _git_result(returncode=0, stdout=""),
            _git_result(returncode=0, stdout="https://github.com/our-ark/oneagent.agent.git"),
        ]
        ...
        self.assertEqual(
            result.compare_url,
            "https://github.com/our-ark/oneagent.agent/compare/main...feature/oneagent?expand=1",
        )
    ...
    def test_pr_creates_with_gh(
        ...
        self.assertIn("feature/oneagent", args)
    ...
    def test_pr_reuses_existing_open_pr_after_ambiguous_create_failure(
        ...
        create = MagicMock(
            returncode=1,
            stdout="",
            stderr="a pull request for branch feature/oneagent already exists",
        )
        view = MagicMock(
            returncode=0,
            stdout=json.dumps(
        ...
        self.assertTrue(result.created)
        self.assertEqual(result.url, "https://github.com/our-ark/oneagent/pull/12")
    ...
    def test_pr_appends_evolution_provenance(
        ...
        run.return_value.returncode = 0
        run.return_value.stdout = "https://github.com/our-ark/oneagent/pull/12\n"
        run.return_value.stderr = ""
    ...
    def test_closes_pull_request_with_comment(
        ...
        run_git.return_value = _git_result(returncode=0, stdout="https://github.com/our-ark/oneagent.git")
        run.return_value.returncode = 0
        ...
        self.assertEqual(result.url, "https://github.com/our-ark/oneagent/pull/2")
        self.assertEqual(
    ...
    def test_inspects_pull_request_merge_evidence(
        ...
        result = inspect_pull_request_merge(
            "https://github.com/our-ark/oneagent/pull/12",
            ROOT,
        )
    ...
    def test_parses_and_normalizes_full_github_pull_request_url(self) -> None:
        target = parse_pull_request_target(
            "https://github.com/our-ark/oneagent/pull/12/?tab=checks"
        )

        self.assertEqual(target.reference, "https://github.com/our-ark/oneagent/pull/12")
        self.assertEqual(target.number, 12)
        self.assertEqual(target.repository, "our-ark/oneagent")

    def test_rejects_non_pull_request_target(self) -> None:
        for target in ("", "0", "feature", "https://github.com/our-ark/oneagent/issues/12"):
    ...
    def test_numeric_target_is_inspected_in_current_repository(
        ...
        self.assertEqual(result.repository, "our-ark/oneagent")
    ...
    def test_lists_open_pull_requests_without_mutation(
        ...
        self.assertEqual(pull_requests[0].author, "oneagent")
    ...
    def test_merges_inspected_head_with_supported_default_method(
        ...
        self.assertEqual(
            ...
            [
                ...
                "--method",
                "PUT",
                "repos/our-ark/oneagent/pulls/12/merge",
                "--input",
                "-",
            ],
        )
