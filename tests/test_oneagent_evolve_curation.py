...
from oneagent.evolution.curation import curation_index_path, load_curations
from oneagent.evolution.events import load_evolve_events, record_evolve_event
from oneagent.tasks.events import record_task_event
...
class OneagentEvolveCurationTests(unittest.TestCase):
    def test_all_sources_enter_bounded_curation_with_unchanged_provenance(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            with patch("oneagent.evolution.core.evolve_report", return_value=report):
    ...
    def test_duplicate_and_resolved_suggestions_are_auditable_but_do_not_remove(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            _record_completed_work(
                ...
                result="Implemented focused experience validation.\nFiles:\n- src/oneagent/evolution/core.py",
                pr_url="https://github.com/our-ark/oneagent/pull/19",
                promoted=True,
            )
            proposal = propose_evolve(
                root,
    ...
    def test_direct_merged_candidate_link_supports_already_resolved_evidence(self) -> None:
                    ...
                    remove=[
                        {
                            ...
                            "evidence_refs": ["task:17", "pr:https://github.com/our-ark/oneagent/pull/19", "merge:e536d32"],
                        }
                    ]
                ),
            )
        ...
        self.assertEqual(
            ...
            (
                "task:17",
                "pr:https://github.com/our-ark/oneagent/pull/19",
                "merge:e536d32",
            ),
        )
    ...
    def test_semantic_resolution_without_direct_link_and_supersession_are_allowed(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            _record_completed_work(
                ...
                pr_url="https://github.com/our-ark/oneagent/pull/22",
                promoted=True,
        ...
        for label, event, runtime_reason, promoted in (
            ...
            with self.subTest(label=label), TemporaryDirectory() as temp:
                ...
                _record_completed_work(
                    ...
                    pr_url="https://github.com/our-ark/oneagent/pull/31",
                    event=event,
