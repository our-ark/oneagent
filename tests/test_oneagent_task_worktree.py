...
from oneagent.vcs_tools import VcsError
...
class TaskWorktreeTests(unittest.TestCase):
    ...
    def test_existing_branch_publish_worktree_does_not_switch_resident(self) -> None:
        with TemporaryDirectory() as temp:
            source, resident = _create_agent_worktree(Path(temp))
            _git(source, "branch", "oneagent/existing", "main")
            ...
            self.assertEqual(_git(task.path, "branch", "--show-current"), "oneagent/existing")
            self.assertEqual(
            ...
            self.assertFalse(task.path.exists())
            self.assertEqual(
                _git(source, "branch", "--list", "oneagent/existing"),
                "oneagent/existing",
            )
...
def _create_agent_worktree(base: Path) -> tuple[Path, Path]:
    ...
    _git(source, "worktree", "add", "-b", "agent/oneagent-gary", str(resident), "main")
