...
class OneAgentPortableInstallTests(unittest.TestCase):
    ...
    def test_wheel_install_completes_profile_task_with_independent_packages(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            environment = {
                **os.environ,
                "ONEAGENT_CODEX_BIN": str(codex),
                "ONEAGENT_PYTHON": sys.executable,
                "ONEAGENT_TEST_COMMAND": f'{sys.executable} -c "pass"',
                "PIP_NO_INDEX": "1",
        ...
        self.assertEqual(result["oneagent_version"], "0.6.1")
        self.assertEqual(
...
def _write_chat_provider_package(root: Path) -> None:
    ...
    (root / "pyproject.toml").write_text(
        textwrap.dedent(
            """
            [project]
            name = "oneagent-portable-chat-provider"
            version = "0.0.1"
...
def _write_vcs_provider_package(root: Path) -> None:
    ...
    (root / "pyproject.toml").write_text(
        textwrap.dedent(
            """
            [project]
            name = "oneagent-portable-vcs-provider"
            version = "0.0.1"
            requires-python = ">=3.11"
...
def _write_profile_package(root: Path) -> None:
    ...
    (root / "pyproject.toml").write_text(
        textwrap.dedent(
            """
            [project]
            name = "oneagent-portable-researcher-profile"
            version = "0.0.1"
            requires-python = ">=3.11"
...
def _write_extension_package(root: Path) -> None:
    ...
    (root / "pyproject.toml").write_text(
        textwrap.dedent(
            """
            [project]
            name = "oneagent-portable-notes-extension"
            version = "0.0.1"
            requires-python = ">=3.11"
...
_INSTALLED_TASK_SCRIPT = textwrap.dedent(
    ...
    from oneagent.conformance import CONFORMANCE_API_VERSION
    from oneagent.extensions import AGENT_EXTENSION_API_VERSION
    ...
    (root / "body.yaml").write_text(
        resources.files("oneagent").joinpath("body.yaml").read_text(
            encoding="utf-8"
        ),
        encoding="utf-8",
    )
    ...
    print(json.dumps({
        ...
        "forge": forge.name,
        "oneagent_version": version("oneagent"),
        "agent_identity_schema_id": agent_identity_schema()["$id"],
        "provider_kit_version": version("our-ark-provider-kit"),
        "chat_provider_version": version("oneagent-portable-chat-provider"),
        "vcs_provider_version": version("oneagent-portable-vcs-provider"),
        "profile": profile.name,
        "profile_version": version("oneagent-portable-researcher-profile"),
        "extension": extensions[0].name,
        "extension_version": version("oneagent-portable-notes-extension"),
        ...
        "workflow_state_isolated": (
            workflow.root != root
            and not (root / ".oneagent" / "task_queue.json").exists()
        ),
        ...
        "extension_state_namespaced": (
            root / ".oneagent" / "extensions" / "notes" / "notes.txt"
        ).is_file(),
        "extension_last_event": (
            root / ".oneagent" / "extensions" / "notes" / "last-event.txt"
