...
class OneagentPromptAppendTests(unittest.TestCase):
    ...
    def test_repository_handoff_note_warns_not_to_assume_landing(self) -> None:
        note = repository_handoff_note(
            "oneagent/turn-readme",
            "https://github.com/our-ark/oneagent/pull/1",
            "agent/oneagent-gary",
            "trunk",
        )

        self.assertIn("oneagent/turn-readme", note)
        self.assertIn("Do not assume the review was landed.", note)
        self.assertIn("The resident workspace is `agent/oneagent-gary`.", note)
