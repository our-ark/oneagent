...
from oneagent.tasks.events import (
...
class OneAgentTaskQueueTests(unittest.TestCase):
    ...
    def test_retry_preserves_recoverable_branch_and_reconciled_pr(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            retried = retry_failed_task(
                ...
                reconciled_result=(
                    ...
                    "https://github.com/our-ark/oneagent/pull/13"
                ),
            )

        self.assertEqual(retried.branch_name, "oneagent/task-1")
        self.assertEqual(
        ...
        self.assertEqual(retried.remote_branch, "oneagent/task-1")
...
""",
    ...
    def test_complete_running_task_records_pr_urls_in_history(self) -> None:
        ...
        self.assertEqual(status.history[-1].id, queued.id)
        self.assertEqual(status.history[-1].status, "completed")
        self.assertEqual(status.history[-1].pr_urls, ("https://github.com/our-ark/oneagent/pull/3",))
    ...
    def test_reconciliation_fences_same_timestamp_lease_renewal(self) -> None:
        ...
        with TemporaryDirectory() as temp, patch(
            "oneagent.tasks.queue.current_time",
    ...
    def test_dead_worker_recovery_preserves_task_worktree_metadata(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            with patch("oneagent.tasks.queue.os.kill", side_effect=ProcessLookupError):
                recovered = recover_interrupted_task(root)
        ...
        self.assertEqual(recovered.branch_name, "oneagent/task-1")
