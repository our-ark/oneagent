...
from oneagent.lineage.assessment import assess_lineage_inbox
from oneagent.commands import inherit_command
from oneagent.lineage.lifecycle import (
    lineage_context_source,
    reconcile_lineage_adoptions,
)
from oneagent.lineage.config import lineage_settings
...
class OneAgentLineageTests(unittest.TestCase):
    ...
    def test_resolve_lineage_reads_parent_metadata_at_immutable_birth_commit(self) -> None:
        ...
        self.assertEqual(client.remote_parent_calls, [("our-ark/oneagent", parent_commit)])
    ...
    def test_refresh_all_ancestors_stores_inbox_candidates(self) -> None:
        ...
        self.assertEqual(inbox_ids, {"our-ark/oneagent#32", "our-ark/lucy#7", "our-ark/lucy#8"})
    ...
    def test_refresh_stores_direct_commit_changes(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            candidate = find_inbox_candidate("oneagent-direct", root)
        ...
        self.assertEqual(candidate.merge_commit, "oneagent-direct-sha")
        ...
        self.assertIn("Commit: oneagent-direct-sha", format_candidate(candidate))
    ...
    def test_missing_diff_is_retried_after_the_discovery_cursor_advances(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            first = find_inbox_candidate("our-ark/oneagent#32", root)
            refresh_lineage_inbox(root, scope="parent", client=client)
            retried = find_inbox_candidate("our-ark/oneagent#32", root)
    ...
    def test_refresh_parent_only_stores_direct_parent_candidates(self) -> None:
        ...
        self.assertEqual(inbox_ids, ["our-ark/oneagent#32"])
    ...
    def test_cursor_is_not_advanced_when_scan_limit_cannot_reach_it(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            root = _root_with_parent(Path(temp))
            config = root / ".oneagent" / "config.yaml"
            config.parent.mkdir()
            config.write_text(
                "lineage:\n  scan_limit: 20\n",
                encoding="utf-8",
        ...
        self.assertEqual(payload["latest_heads"]["our-ark/oneagent"], "base-sha")
    ...
    def test_parent_inherit_report_excludes_grandparent_candidates(self) -> None:
        ...
        self.assertIn("our-ark/oneagent#32", formatted)
    ...
    def test_parent_inherit_report_keeps_unassessed_changes_visible(self) -> None:
        ...
        self.assertEqual([candidate.id for candidate in stored], ["our-ark/oneagent#99"])
        self.assertEqual(stored[0].relevance, "unassessed")
        self.assertIn("Awaiting assessment:", formatted)
        self.assertIn("our-ark/oneagent#99 Update README wording", formatted)
    ...
    def test_ignore_hides_candidate_from_pending_inbox(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            ignored = mark_inbox_candidate("our-ark/oneagent#32", "ignored", root, note="not needed")
    ...
    def test_refresh_preserves_reviewed_candidate_status(self) -> None:
        with TemporaryDirectory() as temp:
            root = _root_with_parent(Path(temp))
            refresh_lineage_inbox(root, scope="parent", client=FakeLineageClient())
            mark_inbox_candidate("our-ark/oneagent#32", "ignored", root, note="not needed")
    ...
    def test_stored_inherit_inbox_does_not_refresh(self) -> None:
        ...
        self.assertIn("our-ark/oneagent#32", reply)
    ...
    def test_landed_review_is_required_before_linked_change_becomes_adopted(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            link_inbox_candidate("our-ark/oneagent#32", 7, root)
            task = TaskJob(
                id=7,
                chat_id=42,
                text="adapt",
                ...
                status="completed",
                context_source=lineage_context_source("our-ark/oneagent#32"),
                review_url="https://github.com/our-ark/oneagent/pull/77",
                review_urls=("https://github.com/our-ark/oneagent/pull/77",),
            )
            ...
            linked = find_inbox_candidate("our-ark/oneagent#32", root)
            landed_result = reconcile_lineage_adoptions(
                root,
                (task,),
                review=FakeReviewProvider(state="landed"),
            )
            adopted = find_inbox_candidate("our-ark/oneagent#32", root)

        assert linked is not None
        assert adopted is not None
        self.assertEqual(open_result.adopted_ids, ())
        self.assertEqual(linked.status, "linked")
        self.assertEqual(landed_result.adopted_ids, ("our-ark/oneagent#32",))
...
class FakeLineageClient:
            ...
            return [
                {
                    ...
                    "mergeCommit": {"oid": "oneagent-merge"},
    ...
    def pr_files(self, repo: str, number: int) -> tuple[str, ...]:
        if repo == "our-ark/oneagent":
            return ("src/oneagent/app/core.py",)
        if number == 7:
            return ("src/oneagent/immune.py",)
        return ("README.md",)
...
class DirectCommitLineageClient(FakeLineageClient):
    ...
    def commits(self, repo: str, branch: str, limit: int = 20) -> list[dict]:
        return [
            {
                "sha": "oneagent-direct-sha",
                "html_url": "https://github.com/our-ark/oneagent/commit/oneagent-direct-sha",
                "commit": {
    ...
    def commit_files(self, repo: str, sha: str) -> tuple[str, ...]:
        return ("src/oneagent/skills/learn/SKILL.md",)


class LowRelevanceLineageClient(FakeLineageClient):
    def latest_commit(self, repo: str, branch: str) -> str:
        return "oneagent-docs"
        ...
        return [
            {
                ...
                "labels": [],
                "mergedAt": "2026-06-17T03:00:00Z",
                "mergeCommit": {"oid": "oneagent-docs"},
                "url": "https://github.com/our-ark/oneagent/pull/99",
            }
        ]
        ...
        return [
            {
                ...
                "url": "https://github.com/our-ark/oneagent/pull/44",
            }
        ]
...
def _commit_payload(sha: str) -> dict:
    return {
        "sha": sha,
        "html_url": f"https://github.com/our-ark/oneagent/commit/{sha}",
        "commit": {
