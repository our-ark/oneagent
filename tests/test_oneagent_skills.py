...
from oneagent.identity import _parse_oneagent_yaml
from oneagent.skills import (
    PublishedSource,
    SkillsError,
    _parse_simple_yaml,
    _published_text,
...
class OneagentSkillsTests(unittest.TestCase):
    ...
    def test_body_skill_versions_match_skill_manifests(self) -> None:
        body = _parse_oneagent_yaml(
            (ROOT / "src" / "oneagent" / "body.yaml").read_text(encoding="utf-8")
        )
    ...
    def test_skills_command_can_inspect_explicit_local_agent_path(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = skills_command(f"skills {lucy}", oneagent, prefix="")
    ...
    def test_skills_command_reads_named_agent_from_github_main(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory) / "instances" / "oneagent-gary"
            with patch(
                "oneagent.skills.catalog.resolve_published_source",
                return_value=_published_source("lucy"),
            ):
                with patch(
                    "oneagent.skills.catalog._published_text",
    ...
    def test_skills_command_keeps_named_agent_independent_from_local_checkout(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            workspace = Path(directory)
            oneagent = workspace / "instances" / "oneagent-gary"
            adam = workspace / "adam"
            ...
            with patch(
                ...
                with patch(
                    ...
                    output = skills_command("skills adam", oneagent, prefix="")

        self.assertIn("Adam skills:", output)
        self.assertIn("github-main", output)
        self.assertNotIn("local-dirty", output)

    @patch("oneagent.skills.catalog.load_provider")
