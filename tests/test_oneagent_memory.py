...
from oneagent.memory.paths import long_term_memory_path
from oneagent.memory.prompt import memory_for_prompt
from oneagent.memory.store import apply_memory_candidates, ensure_long_term_memory, forget_memory, memory_status, remember_memory
...
class OneAgentMemoryTests(unittest.TestCase):
    ...
    def test_prompt_loads_self_from_an_isolated_state_redirect(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(
                os.environ,
                {
                    "ONEAGENT_STATE_REDIRECT_ROOT": str(root),
                    "ONEAGENT_STATE_HOME": str(state_home),
                },
            ):
                prompt_memory = memory_for_prompt(root=root)
    ...
    def test_memory_status_is_readable(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("body identity: rendered from src/oneagent/body.yaml", memory_status(root))
