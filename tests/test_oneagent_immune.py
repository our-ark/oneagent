...
from oneagent.formatting import doctor_output_excerpt
from oneagent.immune import (
    MAX_OUTPUT_CHARS,
...
from oneagent.providers.registry import ProviderError
from oneagent.validation_environment import (
...
def _check(
    ...
    *,
    ...
    from oneagent.immune import DoctorCheckResult
...
class OneAgentImmuneTests(unittest.TestCase):
    ...
    @patch("oneagent.immune._build_backend_check")
    @patch("oneagent.immune.load_provider")
    @patch("oneagent.immune.subprocess.run")
    ...
    @patch("oneagent.immune._memory_storage_check")
    @patch("oneagent.immune._vcs_workspace_check")
    @patch("oneagent.immune._forge_provider_check")
    @patch("oneagent.immune._runtime_provider_check")
    @patch("oneagent.immune._run_check")
    @patch("oneagent.immune._python_runtime_check")
    def test_task_doctor_uses_resident_root_for_operational_state(
        ...
        with TemporaryDirectory() as temporary:
            ...
            worktree.mkdir()
            resident.mkdir()
            with patch.dict("os.environ", {"ONEAGENT_TEST_COMMAND": "true"}):
    ...
    def test_runs_default_test_command(
        ...
        self.assertIn("import oneagent.cli", result.command)
        self.assertIn("import oneagent.providers", result.command)
        self.assertIn("import oneagent.lineage", result.command)
        self.assertIn("import oneagent.learn", result.command)
        self.assertIn("import oneagent.skills", result.command)
        self.assertIn("import oneagent.app.core", result.command)
    ...
    def test_empty_configured_test_command_fails_without_crashing(
        ...
        with patch.dict("os.environ", {"ONEAGENT_TEST_COMMAND": ""}):
            result = run_immune_system(ROOT)
    ...
    def test_timeout_is_reported_as_failed_check(
        ...
        with patch.dict("os.environ", {"ONEAGENT_DOCTOR_TIMEOUT_SECONDS": "1"}):
            result = run_immune_system(ROOT)
...
"""
    ...
    def test_diagnoses_missing_imports(self) -> None:
        output = "ModuleNotFoundError: No module named 'oneagent.telegram_client'"

        diagnosis = diagnose_output(output, passed=False)

        self.assertEqual(diagnosis.summary, "Import smoke failed: missing module oneagent.telegram_client.")
        self.assertIn("stale module path", diagnosis.suggested_action)

    def test_diagnoses_stale_import_names(self) -> None:
        output = "ImportError: cannot import name 'missing_symbol' from 'oneagent.example'"

        diagnosis = diagnose_output(output, passed=False)

        self.assertEqual(
            diagnosis.summary,
            "Import smoke failed: cannot import missing_symbol from oneagent.example.",
        )
    ...
    @patch("oneagent.immune.ensure_validation_environment")
    @patch("oneagent.immune.existing_validation_environment", return_value=None)
    @patch("oneagent.immune._build_backend_check")
    ...
    @patch(
        "oneagent.immune.ensure_validation_environment",
...
"""
    ...
    def test_state_storage_detects_and_preserves_corrupt_json(self) -> None:
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            state_file = root / ".oneagent" / "task_queue.json"
    ...
    def test_state_storage_detects_and_preserves_corrupt_artifact_journal(self) -> None:
        with TemporaryDirectory() as temporary:
            root = Path(temporary)
            journal = root / ".oneagent" / "artifacts" / "task_events.jsonl"
    ...
    @patch("oneagent.brain.resolve_codex_executable")
    @patch("oneagent.immune.subprocess.run")
    ...
    @patch("oneagent.immune.provider_name", side_effect=ProviderError("bad provider config"))
