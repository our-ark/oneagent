...
from oneagent.automatic_learning import learning_artifact_path, learning_index_path, record_learning_artifact
...
class OneagentAutomaticLearningTests(unittest.TestCase):
    def test_records_skill_learning_artifact_index_and_markdown(self) -> None:
            ...
            artifact = record_learning_artifact(
                ...
                result="\n".join(
                    [
                        ...
                        "- src/oneagent/skills/cron/skill.yaml",
                    ]
                ),
                root=root,
                task_id=7,
        ...
        self.assertEqual(payload["pr_urls"], ["https://github.com/our-ark/oneagent/pull/42"])
        self.assertEqual(payload["changed_files"], ["src/oneagent/skills/cron/SKILL.md", "src/oneagent/skills/cron/skill.yaml"])
        self.assertEqual(payload["skill_names"], ["cron"])
        self.assertIn("## Inheritance Notes", markdown)
        self.assertIn("Skills: cron", markdown)
        self.assertIn("src/oneagent/skills/cron/SKILL.md", markdown)
    ...
    def test_skips_non_skill_changes(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            artifact = record_learning_artifact(
                load_identity(),
                request="fix a bug",
                result="\n".join(["Files:", "- src/oneagent/telegram/bot.py"]),
                root=root,
                task_id=3,
                command="/task",
            )
