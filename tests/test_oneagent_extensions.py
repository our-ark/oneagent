...
from oneagent.extensions.events import load_extension_task_event_receipts
from oneagent.extensions.schedules import (
    claim_due_extension_schedules,
    find_extension_schedule,
)
from oneagent.extensions import registry as extension_registry
from oneagent.identity import load_identity
from oneagent.logs import system_log_path
from oneagent.profiles import AgentProfile, CommandSpec, LifecycleHooks
...
class OneagentExtensionTests(unittest.TestCase):
    ...
    def test_extension_command_uses_namespaced_storage_and_shared_workflow(self) -> None:
        ...
        with TemporaryDirectory() as temp:
            ...
            self.assertEqual(
                ...
                root.resolve() / ".oneagent" / "extensions" / "manager",
            )
            self.assertEqual(
                storage_layouts[0].artifacts,
                root.resolve() / ".oneagent" / "artifacts" / "extensions" / "manager",
            )
            self.assertTrue(
    ...
    def test_extension_workflow_round_trips_structured_request_data(self) -> None:
        metadata = {
            ...
            "revision": 3,
            "policy": {"review": True, "owners": ["gary", "oneagent"]},
        }
        artifact_refs = (
            ExtensionArtifactReference(
                "project-spec",
