...
from oneagent.automatic_learning import learning_dir
from oneagent.config import config_path
from oneagent.cron import cron_path
from oneagent.evolution.curation import curation_index_path
from oneagent.evolution.core import evolve_brainstorm_schedule_path
from oneagent.evolution.events import evolve_event_path
from oneagent.extensions import ExtensionTaskEvent, extension_storage
from oneagent.extensions.events import (
    acknowledge_extension_task_event,
    extension_task_event_receipt_path,
)
from oneagent.logs import conversation_log_dir, log_system_event
from oneagent.memory.paths import memory_dir
from oneagent.paths import (
    artifact_path,
    repo_root,
    private_state_path,
    software_body_path,
...
from oneagent.storage import STORAGE_API_VERSION, StorageLayout, StorageLayoutError
from oneagent.tasks.events import load_task_events, task_event_path
...
class OneagentStorageTests(unittest.TestCase):
    def test_default_layout_separates_body_state_and_artifacts(self) -> None:
        ...
        self.assertEqual(layout.private_state, root.resolve() / ".oneagent")
        self.assertEqual(layout.artifacts, root.resolve() / ".oneagent" / "artifacts")
        self.assertTrue(layout.contains("software-body", root / "src" / "agent.py"))
        self.assertTrue(layout.contains("private-state", root / ".oneagent" / "config.yaml"))
        self.assertTrue(
            layout.contains("artifacts", root / ".oneagent" / "artifacts" / "task.json")
        )
        self.assertFalse(
            layout.contains("software-body", root / ".oneagent" / "config.yaml")
        )
        self.assertFalse(
            layout.contains(
                "private-state",
                root / ".oneagent" / "artifacts" / "task.json",
            )
        )
    ...
    def test_classified_paths_reject_escape_and_cross_boundary_access(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                ...
                root.resolve() / ".oneagent" / "config.yaml",
            )
            self.assertEqual(
                artifact_path("task_events.jsonl", root),
                root.resolve() / ".oneagent" / "artifacts" / "task_events.jsonl",
            )
            self.assertEqual(
                software_body_path("src/oneagent", root),
                root.resolve() / "src" / "oneagent",
            )
    ...
    def test_explicit_sibling_roots_are_not_captured_by_ancestor_repo(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            ancestor_state_exists = ancestor.joinpath(".oneagent").exists()
        ...
        self.assertEqual(first_queue, first.resolve() / ".oneagent" / "task_queue.json")
        self.assertEqual(second_queue, second.resolve() / ".oneagent" / "task_queue.json")
        self.assertEqual(
            first_events,
            first.resolve() / ".oneagent" / "artifacts" / "task_events.jsonl",
        )
        self.assertEqual(
            second_events,
            second.resolve() / ".oneagent" / "artifacts" / "task_events.jsonl",
        )
        self.assertEqual(
            first_receipts,
            first.resolve()
        ...
        self.assertFalse(first_queue.is_relative_to(ancestor / ".oneagent"))
    ...
    def test_implicit_root_discovery_still_finds_ancestor_repo(self) -> None:
        ...
        self.assertEqual(layout.private_state, ancestor.resolve() / ".oneagent")
    ...
    def test_state_redirect_matches_only_the_explicit_software_body(self) -> None:
        with TemporaryDirectory() as directory, TemporaryDirectory() as redirected:
            ...
            with patch.dict(
                os.environ,
                {
                    "ONEAGENT_STATE_REDIRECT_ROOT": str(configured),
                    "ONEAGENT_STATE_HOME": redirected,
                },
            ):
    ...
    def test_legacy_artifact_events_remain_readable(self) -> None:
        with TemporaryDirectory() as directory:
            root = Path(directory)
            legacy = root / ".oneagent" / "task_events.jsonl"
