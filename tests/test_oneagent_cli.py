...
from oneagent.cli import ADMIN_COMMANDS, _repl, admin_help, main
from oneagent.config import read_section
from oneagent.immune import DoctorCheckResult, DoctorDiagnosis
...
class OneAgentCliTests(unittest.TestCase):
    ...
    @patch("oneagent.cli.mission_command", return_value="Mission: ok")
    ...
    @patch("oneagent.cli.lineage_command", return_value="Ancestors: no direct parent configured.")
    ...
    @patch("oneagent.cli.update_from_authoritative")
    ...
    def test_setup_interactively_saves_token_without_printing_secret(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("bin/oneagent-daemon start", output)
    ...
    def test_setup_token_command_shows_next_step(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("oneagent.cli.Path.cwd", return_value=root), redirect_stdout(output):
            ...
            self.assertIn("bin/oneagent-daemon start", output.getvalue())
    ...
    def test_setup_chat_command_saves_chat_lock_from_direct_cli(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("bin/oneagent-daemon restart", output.getvalue())
    ...
    def test_setup_ancestor_writes_repo_side_lineage_parent(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            output = _run_repl_commands(
                "setup ancestor https://github.com/our-ark/oneagent",
                "setup ancestor show",
                "exit",
                root=root,
            )
            ...
            self.assertIn("Parent: Oneagent (our-ark/oneagent@main)", output)
            self.assertIn("Lineage parent: Oneagent (our-ark/oneagent@main)", output)
            self.assertIn("repo-side lineage metadata", output)
            self.assertIn("name: Oneagent", lineage.read_text(encoding="utf-8"))
            self.assertIn("repo: our-ark/oneagent", lineage.read_text(encoding="utf-8"))
    ...
    def test_setup_ancestor_rejects_non_link_forms(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(output.count("Use bin/oneagent setup ancestor <repo-url>."), 3)
    ...
    @patch("oneagent.cli.run_immune_system")
    ...
    def test_state_command_validates_and_dry_runs_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse((root / ".oneagent").exists())
    ...
    def test_state_command_applies_migration(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = root / ".oneagent" / "state_manifest.json"
