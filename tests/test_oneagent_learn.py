...
class OneagentLearnTests(unittest.TestCase):
    ...
    def test_loads_validated_skill_snapshot_from_one_revision(self) -> None:
        with patch("oneagent.learn.resolve_published_source", return_value=_source()):
            with patch("oneagent.learn._published_text", side_effect=_published_text) as read:
    ...
    def test_refuses_missing_skill(self) -> None:
        with patch("oneagent.learn.resolve_published_source", return_value=_source()):
            with patch("oneagent.learn._published_text", side_effect=_published_text):
    ...
    def test_refuses_hidden_skill(self) -> None:
        ...
        with patch("oneagent.learn.resolve_published_source", return_value=_source()):
            with patch("oneagent.learn._published_text", side_effect=hidden_text):
    ...
    def test_routes_direct_parent_to_inheritance(self) -> None:
        with TemporaryDirectory() as temp:
            ...
            with patch(
                "oneagent.learn.resolve_published_source",
