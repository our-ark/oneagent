...
from oneagent import load_body_identity, load_identity
...
class OneAgentIdentityTests(unittest.TestCase):
    def test_loads_identity_from_body(self) -> None:
        ...
        self.assertEqual(identity.body.source_path, "src/oneagent")
    ...
    def test_body_yaml_is_the_only_versioned_body_identity_source(self) -> None:
        self.assertTrue((ROOT / "src" / "oneagent" / "body.yaml").exists())
        self.assertFalse((ROOT / "src" / "oneagent" / "identity.yaml").exists())
    ...
    def test_identity_declares_skills(self) -> None:
        text = (ROOT / "src" / "oneagent" / "body.yaml").read_text(encoding="utf-8")
    ...
    def test_legacy_identity_yaml_remains_readable_during_migration(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            legacy = root / "src" / "oneagent" / "identity.yaml"
            legacy.parent.mkdir(parents=True)
            legacy.write_text(
                (ROOT / "src" / "oneagent" / "body.yaml")
                .read_text(encoding="utf-8")
                .replace("body_file: src/oneagent/body.yaml", "identity_file: src/oneagent/identity.yaml"),
                encoding="utf-8",
            )
