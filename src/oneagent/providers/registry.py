...
from oneagent.runtime_dependencies import load_runtime_dependencies
...
CORE_PROVIDER_MODULES = (
    "oneagent.providers.runtime",
    "oneagent.providers.vcs",
    "oneagent.providers.forge",
)
...
def provider_name(kind: ProviderKind, root: Path | None = None) -> str:
    _require_kind(kind)
    _load_provider_plugins(root)
    env_name = f"ONEAGENT_{kind.upper()}_PROVIDER"
