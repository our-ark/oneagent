...
from oneagent.memory.paths import clean_text, now as current_time
from oneagent.paths import artifact_path, artifact_read_paths
from oneagent.tasks.events import TaskEvent, load_recent_task_outcomes
...
_MEMORY_BLOCK_PATTERN = re.compile(
    r"(?is)\[ONEAGENT_MEMORY_REQUEST\].*?\[/ONEAGENT_MEMORY_REQUEST\]"
)
...
_PRIVATE_STATE_PATTERN = re.compile(r"(?<![\w/])\.oneagent/[^\s,;)\]}]+")
...
_PROTECTED_ACTION_PATTERN = re.compile(
    rf"(?:\b{_ACTION}\b\s+(?:(?:the|our|oneagent'?s|agent'?s)\s+)?\b{_PROTECTED}\b|"
...
def _safe_changed_files(values: Iterable[str]) -> tuple[str, ...]:
    ...
    for value in values:
        ...
        if (
            not cleaned
            or cleaned.startswith(("/", "../", ".oneagent/"))
            or "/../" in cleaned
