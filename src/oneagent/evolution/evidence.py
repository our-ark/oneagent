...
from oneagent.evolution.curation import candidate_scope_is_safe
from oneagent.logs import conversation_log_dirs
from oneagent.memory.paths import clean_text, now as current_time
from oneagent.paths import artifact_path, artifact_read_paths, private_state_path
from oneagent.state import atomic_write, file_transaction, load_json_object
from oneagent.tasks.events import TaskEvent, load_task_events
...
_SETUP_TOKEN_PATTERN = re.compile(
    r"(?i)(\b(?:bin/oneagent\s+)?setup\s+token\s+)[^\s<>]+"
)
    ...
    prompt_input = [
        {
            ...
            "oneagent_reply": _redact_record_text(turn.reply),
        }
        for turn in batch
    ]
    return _run_scan(
