...
from oneagent.profiles.contracts import AgentProfile, ProfileError
...
def load_profile(root: Path | None = None, *, name: str = "") -> AgentProfile:
    selected = name.strip() or os.environ.get("ONEAGENT_PROFILE", "").strip()
    if not selected:
        selected = read_section("agent", root).get("profile", "").strip()
    if not selected:
        return AgentProfile(name="oneagent")
    selected = _profile_name(selected)
    if selected == "oneagent":
        return AgentProfile(name="oneagent")
