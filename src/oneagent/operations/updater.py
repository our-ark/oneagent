...
from oneagent.channel import load_channel_lifecycle, provider_label
from oneagent.formatting import format_doctor_result
from oneagent.immune import DoctorCheckResult, DoctorDiagnosis, ImmuneResult
from oneagent.providers import as_repository_provider
from oneagent.providers.contracts import (
    RepositoryProvider,
    RepositoryProviderError,
)
from oneagent.providers.registry import provider_name
...
def run_update_doctor(root: Path) -> ImmuneResult:
    ...
    python = environment.get("ONEAGENT_PYTHON") or sys.executable
    try:
        completed = subprocess.run(
            [python, "-m", "oneagent.operations.update_doctor"],
            cwd=root,
            env=environment,
            text=True,
            capture_output=True,
...
def _doctor_runner_failure(detail: str) -> ImmuneResult:
    check = DoctorCheckResult(
        ...
        passed=False,
        command=f"{sys.executable} -m oneagent.operations.update_doctor",
        output=detail,
