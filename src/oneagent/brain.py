...
from oneagent.identity import Identity
from oneagent.last_codex_input import record_last_codex_input
from oneagent.memory.prompt import memory_for_prompt
from oneagent.prompt_append import startup_context_note
...
from oneagent.runtime import ACTION_SANDBOX_READ_ONLY, WORKSPACE_WRITE_SANDBOX
from oneagent.tasks.config import task_timeout_seconds
...
_TOKEN_USAGE: ContextVar[TokenUsage] = ContextVar("oneagent_token_usage", default=TokenUsage())
...
def model_summary(root: Path | None = None) -> str:
    env_model = os.environ.get("ONEAGENT_CODEX_MODEL", "").strip()
    env_reasoning = os.environ.get("ONEAGENT_CODEX_REASONING_EFFORT", "").strip()
    oneagent_model = _oneagent_model(root)
    oneagent_reasoning = _oneagent_reasoning_effort(root)
    ...
    if env_reasoning:
        lines.extend(
            [
                ...
                "Reasoning source: ONEAGENT_CODEX_REASONING_EFFORT",
            ]
        )
    elif oneagent_reasoning:
        lines.extend(
            [
    ...
    *,
    ...
    with tempfile.NamedTemporaryFile("r+", encoding="utf-8", delete=True) as output:
        ...
        timeout = control.timeout_seconds or int(
            os.environ.get("ONEAGENT_CODEX_TIMEOUT", task_timeout_seconds(state_root))
        )
        if (
...
def _configured_reasoning_effort(root: Path | None = None) -> str:
    ...
    return _oneagent_reasoning_effort(root)
...
def _configured_model(root: Path | None = None) -> str:
    ...
    if env_model:
        return env_model
    return _oneagent_model(root)


def _oneagent_model(root: Path | None = None) -> str:
    if root is None:
        return ""
    return read_section("codex", root).get("model", "").strip()


def _oneagent_reasoning_effort(root: Path | None = None) -> str:
    if root is None:
        return ""
...
def _run_with_progress(
    ...
    interval = int(os.environ.get("ONEAGENT_PROGRESS_INTERVAL", DEFAULT_PROGRESS_INTERVAL_SECONDS))
...
def resolve_codex_executable(root: Path | None = None) -> CodexExecutableResolution:
    configured = os.environ.get("ONEAGENT_CODEX_BIN", "").strip()
    if configured:
        return resolve_codex_executable_value(configured, "ONEAGENT_CODEX_BIN")

    oneagent_configured = read_section("codex", root).get("executable", "").strip()
