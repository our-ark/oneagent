...
history from `.oneagent/artifacts/task_events.jsonl`, not merely the last event.
...
Evidence settings are stored in `.oneagent/evidence.json`.

Evidence is append-only in:

- `.oneagent/artifacts/evidence.jsonl`
- `.oneagent/artifacts/evidence_scans.jsonl`
...
Curations are appended to `.oneagent/evolve_curations.jsonl`. Candidates are
stored in `.oneagent/evolve_candidates.json`, and proposal, decision, and handoff
events are appended to `.oneagent/artifacts/evolve_events.jsonl`.
